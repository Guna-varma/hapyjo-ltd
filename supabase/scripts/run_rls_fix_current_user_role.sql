-- Run this in Supabase Dashboard → SQL Editor to fix 500 on sites, vehicles, trips, etc.
-- Uses current_user_role() (SECURITY DEFINER) in RLS so role checks don't trigger errors.
-- Run after: run_get_my_profile_rpc.sql (if you use that for profile fetch).
...
CREATE POLICY "Sites read by role"
  ...
  USING ((current_user_role()) IN ('admin', 'owner', 'head_supervisor', 'accountant') OR (id IN (SELECT site_id FROM public.site_assignments WHERE user_id = auth.uid())));
...
CREATE POLICY "Vehicles read by role"
  ...
  USING ((current_user_role()) IN ('admin', 'owner', 'head_supervisor', 'accountant') OR (site_id IN (SELECT site_id FROM public.site_assignments WHERE user_id = auth.uid())));
...
CREATE POLICY "Accountant insert update reports"
  ON public.reports FOR ALL
  USING ((current_user_role()) = 'accountant') WITH CHECK ((current_user_role()) = 'accountant');
...
CREATE POLICY "Surveyor update own surveys"
  ...
  USING ((current_user_role()) = 'surveyor' AND surveyor_id = auth.uid()) WITH CHECK (true);
...
CREATE POLICY "Assistant supervisor update site surveys"
  ON public.surveys FOR UPDATE TO authenticated
  USING ((current_user_role()) = 'assistant_supervisor' AND site_id IN (SELECT site_id FROM public.site_assignments WHERE user_id = auth.uid())) WITH CHECK (true);
...
CREATE POLICY "Machine_sessions read by role"
  ON public.machine_sessions FOR SELECT TO authenticated
  USING (
    (current_user_role()) IN ('admin', 'owner', 'head_supervisor', 'accountant')
    OR ((current_user_role()) = 'assistant_supervisor' AND site_id IN (SELECT site_id FROM public.site_assignments WHERE user_id = auth.uid()))
    OR ((current_user_role()) IN ('driver_truck', 'driver_machine') AND driver_id = auth.uid())
...
CREATE POLICY "Machine_sessions write by drivers and management"
  ON public.machine_sessions FOR ALL TO authenticated
  USING ((current_user_role()) IN ('admin', 'owner', 'head_supervisor', 'assistant_supervisor', 'driver_truck', 'driver_machine'))
  WITH CHECK ((current_user_role()) IN ('admin', 'owner', 'head_supervisor', 'assistant_supervisor', 'driver_truck', 'driver_machine'));
