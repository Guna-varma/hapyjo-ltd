-- Security hardening: approve_assigned_trip_readings() is SECURITY DEFINER (it must
-- update assigned_trips, trips, machine_sessions and vehicles atomically), which
-- means it bypasses RLS. It previously had no caller check, so ANY signed-in user
-- could approve any pending trip via /rest/v1/rpc/... even though the app only
-- offers approval to the site's Assistant Supervisor.
--
-- This adds a guard that mirrors the existing RLS UPDATE policy on assigned_trips
-- ("AS update assigned_trips for own sites") exactly:
--   current_user_role() = 'assistant_supervisor'
--   AND the caller is sites.assistant_supervisor_id for the trip's site.
--
-- No business logic, calculations, or status transitions change; the function body
-- is identical apart from the authorization check inserted after the row lock.
...
CREATE OR REPLACE FUNCTION public.approve_assigned_trip_readings(
  ...
  p_validation_notes text DEFAULT NULL::text,
  p_manual_fuel_override_l numeric DEFAULT NULL::numeric,
...
BEGIN
  ...
  -- Authorization guard (mirrors RLS policy "AS update assigned_trips for own sites").
  IF auth.uid() IS NULL
     OR public.current_user_role()::text <> 'assistant_supervisor'
     OR NOT EXISTS (
       SELECT 1 FROM public.sites s
       WHERE s.id = v_trip.site_id AND s.assistant_supervisor_id = auth.uid()
     )
  THEN
    RAISE EXCEPTION 'not authorized to approve this trip' USING ERRCODE = '42501';
  END IF;
...
-- The anon role has no reason to call this; only signed-in users may.
REVOKE EXECUTE ON FUNCTION public.approve_assigned_trip_readings(text, numeric, numeric, text, numeric, text) FROM anon, public;
