-- Bug fix: approve_assigned_trip_readings() failed for every caller with
--   ERROR: column reference "status" is ambiguous
-- because `status` is both a RETURNS TABLE output column (a PL/pgSQL variable) and
-- a column of public.assigned_trips, and the UPDATE's WHERE clause referenced it
-- unqualified. This is present in the original 20260307153000 definition too, so
-- trip/task approval has been failing in production since then.
--
-- Fix: qualify the two references in that WHERE clause. No other change - the
-- authorization guard from 20260911100000 and all calculations are identical.
-- Safe to re-run.
...
BEGIN
  ...
  WHERE public.assigned_trips.id = v_trip.id
    AND public.assigned_trips.status IN ('TRIP_NEED_APPROVAL', 'TASK_NEED_APPROVAL');
